...
// Analisar dos pontos x e y ser uma certa porcentagem deslocado da extrema esqquerda da tela;
player* player_create(unsigned short largura, unsigned short altura, unsigned short x, unsigned short y, unsigned short max_x, unsigned short max_y)
{
	...
	new_player->x = largura/2; //analisar se precisa de casting
	new_player->y = altura/2;  //analisar se precisa de casting
	new_player->Vx = 0;
	new_player->Vy = 0;
	new_player->estado_fisico = PARADO;
...
/*
	...
	new_player->hp = 5;																																	//Insere o total de pontos de vida de um quadrado (!)
...
void player_move(player *element, char steps, unsigned char trajectory, unsigned short max_x, unsigned short max_y){									//Implementação da função "player_move"
	...
	if (!trajectory){ if ((element->x - steps*player_STEP) - element->side/2 >= 0) element->x = element->x - steps*player_STEP;} 						//Verifica se a movimentação para a esquerda é desejada e possível; se sim, efetiva a mesma
	else if (trajectory == 1){ if ((element->x + steps*player_STEP) + element->side/2 <= max_x) element->x = element->x + steps*player_STEP;}			//Verifica se a movimentação para a direita é desejada e possível; se sim, efetiva a mesma
	else if (trajectory == 2){ if ((element->y - steps*player_STEP) - element->side/2 >= 0) element->y = element->y - steps*player_STEP;}				//Verifica se a movimentação para cima é desejada e possível; se sim, efetiva a mesma
	else if (trajectory == 3){ if ((element->y + steps*player_STEP) + element->side/2 <= max_y) element->y = element->y + steps*player_STEP;}			//Verifica se a movimentação para baixo é desejada e possível; se sim, efetiva a mesma
}

void player_shot(player *element){																														//Implementação da função "player_shot"
	bullet *shot;
